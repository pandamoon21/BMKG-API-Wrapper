# bmkgaw — Pembungkus API Info BMKG
...
| 5 | Feed InaTEWS GCS (`live30event.xml`, CAP) | host feed GCS | — | — |

Endpoint `df/v1/*` + maritim (sumber #1) butuh token statis `API_LOCK` yang
dibawa app. Token **tidak ikut di-commit** — set `BMKGAW_TOKEN` lewat env
(lihat `.env.example`). Tanpa token, `source=auto` otomatis memakai open data
web publik.

> **Atribusi:** BMKG wajib dicantumkan sebagai sumber data (syarat portal
...
# Opsional — tanpa token, source=auto memakai web open-data.
...
| Var | Dibutuhkan | Default | Keterangan |
|---|---|---|---|
| `BMKGAW_TOKEN` | endpoint app `df/v1` + marine | — (kosong) | tanpa ini `auto` → web; isi dari `API_LOCK` APK |
| `BMKGAW_AGENT` | tidak | `infobmkg` | nama agent yang dikirim ke app API (header `AGENT`) |
| `BMKGAW_CACHE_BACKEND` | tidak | `memory` | `memory` atau `sqlite` |
| `BMKGAW_CACHE_DB_URL` | jika `sqlite` | `sqlite:///bmkgaw_cache.db` | lokasi database |

Token (`API_LOCK`) adalah **kredensial statis bersama** yang dikirim app —
client credential, bukan rahasia per-user. Route auth mengirimnya lewat header
`Authorization` (atau `x-api-key-infobmkg` / `x-api-key` / `apikey` — server
menerima semuanya). Token **tidak pernah di-commit**; isi `BMKGAW_TOKEN` di
env lokal (dari baris `API_LOCK` di APK Info BMKG) kalau ingin memakai
endpoint app df/v1 + marine.

## Pilih sumber data (`?source=`)

Endpoint auth-gated (app df/v1 + marine) menerima parameter query `source`:
...
(`/publik/prakiraan-cuaca?adm4=…`, terverifikasi live). Endpoint tanpa padanan
web (`present`, `area/search`, `area/coord`, `marine forecast`) →
`source=web` ditolak 400, dan `auto` tanpa token → 400 minta token.
...
| `GET /weather/present?adm4=…` | app | ✅ | 120 | cuaca saat ini (tanpa web) |
| `GET /weather/legacy?lat=&lon=` | app | — | 300 | ⚠️ deprecated (payload kosong) |
| `GET /area/search?q=` | app | ✅ | 60 | ⚠️ server mengabaikan `q` |
...
| `GET /marine/forecast` | app | ✅ | 1800 | raw ~1.5 MB (codec `fct_code`) |
...
| `GET /earthquake/warning` | gcs | — | 60 | peringatan geo/tsunami aktif |
...
|---|---|---|
| `GET /weather/legacy?lat=&lon=` | `200` tapi `[]` | deprecated — server mengembalikan daftar kosong |
| `GET /area/search?q=` | `200` tapi statis | bug server: `q` diabaikan, selalu daftar Kemayoran — pakai `/area/coord` |
| `GET /warnings/{lokasi}` | `404` | **normal** — tidak ada peringatan aktif untuk kode itu; cek `/warnings` untuk daftar `ID_Kode` valid |
| `GET /radar` · `GET /radar/image` | `403` | nonaktif server-side oleh BMKG (sejak 2026-09-03) — `radar*` di-`Forbidden`; `?lat=&lon=` / `?radar=` tetap 403 |
| `GET /earthquake/warning` | `200` (bisa kosong) | kosong = tidak ada peringatan geo/tsunami aktif (valid) |
...
### Contoh respons (hasil tangkapan live)
...
Catatan: nama field dalam respons berbahasa Indonesia karena itu memang data
asli dari API BMKG — dibiarkan apa adanya.
...
async def main():
    b = BMKG()                          # membaca BMKGAW_TOKEN dari env (opsional)
    g  = await b.quake_latest()         # gempa terbaru (TEWS, tanpa token)
    f  = await b.forecast(adm4="31.71.03.1001")                 # auto: token→app, kosong→web
...
Setiap entri menyimpan `sha256` isi respons; saat TTL kedaluwarsa lalu isinya
berubah, entri diberi `changed=1` (berguna untuk polling — mis. gempa / feed
peringatan dini). Inspeksi: `curl :8000/api/v1/cache/stats`.

## Batas rate & throttle

Open data BMKG membatasi **60 req/menit/IP**. Wrapper memakai throttle global
**1 req/detik** keluar (satu `asyncio.Lock` bersama di semua host dan route),
...
(mengabaikan `q`), `radar*` (403), `translate` (500), `warningcuaca/{loc}`
...
```bash
vercel deploy            # memakai vercel.json + api/index.py (ASGI)
```
...
- Token **tidak pernah di-commit**. `.env.example` hanya berisi placeholder;
  set `BMKGAW_TOKEN` via env atau file `.env` lokal (di-`.gitignore`).
- Header auth dikirim **hanya** ke `api-apps.bmkg.go.id` (app API) — tidak
  pernah ke host open-data.
- Non-root user di Docker; healthcheck bawaan.

## Disclaimer

Data © BMKG. Wrapper ini tidak berafiliasi dengan dan tidak didukung BMKG.
Gunakan sesuai syarat portal terbuka resmi BMKG; cantumkan BMKG sebagai sumber.
